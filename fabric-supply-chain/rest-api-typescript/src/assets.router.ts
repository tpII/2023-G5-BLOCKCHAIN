...
assetsRouter.post(
  ...
  body('Varietal', 'must be a string').notEmpty(),
  body('Owner', 'must be a string').notEmpty(),
  body('Price', 'must be a number').isNumeric(),
  body('Temperature', 'must be a number').isNumeric(),
  body('Humidity', 'must be a number').isNumeric(),
  body('Winery', 'must be a string').notEmpty(),
  body('Year', 'must be a string').notEmpty(),
  body('Latitude', 'must be a number').isNumeric(),
  body('Longitude', 'must be a number').isNumeric(),
...
// Obtener historial de transacciones del asset
assetsRouter.get('/history/:assetId', async (req: Request, res: Response) => {
  const assetId = req.params.assetId;
  logger.debug('Read asset history request received for asset ID %s', assetId);
...
assetsRouter.patch(
  ...
  body('*.Role', 'must be a string').isString(),
  async (req: Request, res: Response) => {
