...
/**
 * Representa el error que ocurre cuando el rol es invalido en una
 * transacción
 */
export class InvalidRoleError extends ContractError {
  constructor(message: string, transactionId: string) {
...
/**
 * Representa el error que ocurre cuando el owner se quiere apropiar de algo
 */
export class InvalidOwnerError extends ContractError {
  constructor(message: string, transactionId: string) {
    super(message, transactionId);
...
const matchInvalidRoleMessage = (message: string): string | null => {
  const invalidRoleRegex = /You don't have the permission WRONG ROLE/g;
  const invalidRoleMatch = message.match(invalidRoleRegex);

  if (invalidRoleMatch !== null) {
    return invalidRoleMatch[0];
  }

  return null;
};

const matchInvalidOwnerMessage = (message: string): string | null => {
  const invalidOwnerRegex = /You don't have the permission WRONG MSPID/g;
  const invalidOwnerMatch = message.match(invalidOwnerRegex);

  if (invalidOwnerMatch !== null) {
    return invalidOwnerMatch[0];
  }
...
export const handleError = (
  ...
  if (isErrorLike(err)) {
    ...
    const invalidRoleMatch = matchInvalidRoleMessage(err.message);
    if (invalidRoleMatch !== null) {
      return new InvalidRoleError(invalidRoleMatch, transactionId);
    }

    const invalidOwnerMatch = matchInvalidOwnerMessage(err.message);
    if (invalidOwnerMatch !== null) {
      return new InvalidOwnerError(invalidOwnerMatch, transactionId);
    }
